...
exports.borrowedBooksByUser = async (req, res) => {
    try {
        const username = req.params.username;

        // Find the user with borrowed books
        const user = await userSchema.findOne({ username });
        ...
        if (!user.borrowed || user.borrowed.length === 0) {
            return res.status(404).json({ msg: "No borrowed books found for this user" });
        }

        // Collect borrowed books data
        const borrowedBooks = await Promise.all(
            user.borrowed.map(async (book) => {
                const bookDetails = await bookSchema.findOne({ ISBN: book.isbn });
                return {
                    isbn: book.isbn,
